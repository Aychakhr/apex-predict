...
def check_ectyper_available():
    try:
        result = subprocess.run("ectyper --help", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=5)
...
def detect_serogroup_with_ectyper(fasta_path):
    try:
        ...
        result = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, timeout=40)
...
st.markdown('<p class="main-title">🦠ApexPredict</p>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">Pathotype Classification, MLST, Serogroup Detection, and Resistance Profiling</p>', unsafe_allow_html=True)
...
if uploaded_file is not None:
    ...
    try:
        ...
        mlst_proc = subprocess.run(mlst_cmd, shell=True, stdout=subprocess.PIPE, universal_newlines=True, timeout=30)
        if mlst_proc.stdout:
    ...
    for database in ["ecoli_vf", "apec"]:
        if database == "ecoli_vf":
            ...
            cmd = f"abadir = '{db_directory}'\nabricate --datadir {db_directory} --db apec {temp_path}"
        
        try:
            proc = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, universal_newlines=True, timeout=60)
            if proc.stdout:
    ...
    try:
        ...
        amr_proc = subprocess.run(amr_cmd, shell=True, stdout=subprocess.PIPE, universal_newlines=True, timeout=60)
        if amr_proc.stdout:
    ...
    with m_col2:
        st.metric(label="Lineage (MLST)", value=f"ST{mlst_st}", delta=risk_label, delta_color="inverse" if is_high_risk else "normal")
    ...
    with m_col4:
        if serogroup in APEC_SEROGROUPS:
            st.metric(label="Serogroup", value=serogroup, delta=serogroup_risk, delta_color="inverse")
        else:
    ...
    with st.expander(" Detailed Serogroup Information", expanded=False):
        ...
        with col2:
            ...
            elif serogroup not in ["Unknown", "Detection Failed", "Timeout", "Error", "ECTyper not available", "Detection Error"]:
